...
// 2. Sign in butonuna basin
       ...
       WebElement dropDown= driver.findElement(By.id("pc_currency"));
        ...
        softAssert.assertEquals(actualData,expectedValue,"secilen option Eurozone degildir.");

        // 10. soft assert kullanarak DropDown listesinin su secenekleri oldugunu test edin

        List<WebElement> tumOpsiyonlar = select.getOptions();
        //option listesi Webelement'lerden olusuyor.
        //expected liste ise String, bunun icin opsiyon listesini String yapmalıyız.
        ...
        for (WebElement each:tumOpsiyonlar) {
            ...
            List<String> expectedOptionsList = Arrays.asList( "Select One", "Australia (dollar)", "Canada (dollar)",
                    "Switzerland (franc)","China (yuan)","Denmark (krone)",
                    "Eurozone (euro)","Great Britain (pound)","Hong Kong (dollar)",
                    "Japan (yen)","Mexico (peso)","Norway (krone)","New Zealand (dollar)",
                    "Sweden (krona)","Singapore (dollar)","Thailand (baht)");

            softAssert.assertEquals(tumOpsiyonlarString,expectedOptionsList,"Liste farklı.");
